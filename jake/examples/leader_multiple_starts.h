...
struct LeaderMultiStart: public ActorBenchmark {
    ...
    void make() {
        ...
        when (make_cown<LeaderMultiStart>(servers, starters)) << [=](acquired_cown<LeaderMultiStart> ld) {
            ...
            std::vector<uint16_t> starts = gen_x_unique_randoms<uint16_t>(starters, servers-1);
            for (uint16_t i = 0; i < starters; i++) {
                Server::send(server_list[starts[i]], Message(0, Candidate));
            }
        };
    }
