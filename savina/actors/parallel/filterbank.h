...
void FirFilter::value(const cown_ptr<FirFilter>& self, uint64_t n) {
  when(self) << [n](acquired_cown<FirFilter> self)  mutable {
    ...
    if (self->is_full) {
      ...
      visit(overloaded {
        [&](const cown_ptr<SampleFilter>& next) { SampleFilter::value(next, sum); },
        [&](const cown_ptr<TaggedForward>& next) { TaggedForward::value(next, sum); }
      }, self->next);
    }
  };
}
...
struct Integrator {
  ...
  static void value(cown_ptr<Integrator> self, uint64_t id, uint64_t n) {
    when(self) << [id, n](acquired_cown<Integrator> self) mutable mutable {
      bool processed = false;
      uint64_t size = self->data.size();
      uint64_t i = 0;
...
struct FirFilter {
  uint64_t length;
  vector<uint64_t> coefficients;
  cown_ptr<SampleFilter> sf_next;
  cown_ptr<TaggedForward> tf_next;
  ...
  FirFilter(uint64_t length, vector<uint64_t> coefficients, cown_ptr<SampleFilter> next):
    length(length), coefficients(coefficients), sf_next(next), index(0), data(length, 0), is_full(false) {}

  FirFilter(uint64_t length, vector<uint64_t> coefficients, cown_ptr<TaggedForward> next):
    length(length), coefficients(coefficients), tf_next(next), index(0), data(length, 0), is_full(false) {}
...
void FirFilter::value(cown_ptr<FirFilter> self, uint64_t n) {
  when(self) << [n](acquired_cown<FirFilter> self)  mutable {
    ...
    if (self->is_full) {
      ...
      if (self->sf_next == nullptr) {
        TaggedForward::value(self->tf_next, sum);
      } else {
        SampleFilter::value(self->sf_next, sum);
      };
    }
  };
}
